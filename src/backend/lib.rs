...
use utils::{validate_caller, get_subaccount_from_principal, get_multiple_subaccounts_from_principal};
...
fn post_upgrade() {
    ...
    RUNTIME_STATE.with(|state| state.borrow_mut().data.genesis_updating = true);
...
fn update_username(user_account: String, user_name: String) -> bool {
    RUNTIME_STATE.with(|state| update_username_impl(user_account, user_name,  &mut state.borrow_mut()))
}
fn update_username_impl(user_account: String, user_name: String, runtime_state: &mut RuntimeState) -> bool {
    let v = runtime_state.data.authorised.clone();
    validate_caller(ic_cdk::caller().to_text(), v);  // is authorised?
    let mut ud: UserData;
    match runtime_state.data.users.get(&user_account){
...
fn update_user_tokens(user_account: String, user_tokens: u32) -> bool {
    RUNTIME_STATE.with(|state| update_user_tokens_impl(user_account, user_tokens,  &mut state.borrow_mut()))
}
fn update_user_tokens_impl(user_account: String, user_tokens: u32, runtime_state: &mut RuntimeState) -> bool {
    let v = runtime_state.data.authorised.clone();
    validate_caller(ic_cdk::caller().to_text(), v);  // is authorised?
    let mut ud: UserData;
    match runtime_state.data.users.get(&user_account){
...
fn get_single_account(input_principal:String, input_subaccount: u32) -> String {
    RUNTIME_STATE.with(|state| get_single_account_impl(input_principal, input_subaccount as u8, &state.borrow()))
}
fn get_single_account_impl(inpt_principal:String, inpt_subaccount:u8, runtime_state: &RuntimeState) -> String {
    let v = runtime_state.data.authorised.clone();
    validate_caller(ic_cdk::caller().to_text(), v);  // is authorised?
...
fn get_multiple_account(input_principal:String, start: u32, get_number: u32) -> Vec<String> {
    RUNTIME_STATE.with(|state| get_multiple_account_impl(input_principal, start as u8, get_number as u8, &state.borrow()))
}
fn get_multiple_account_impl(inpt_principal:String, inpt_start:u8, inpt_number:u8, runtime_state: &RuntimeState) -> Vec<String> {
    let v = runtime_state.data.authorised.clone();
    validate_caller(ic_cdk::caller().to_text(), v);  // is authorised?
...
fn is_genesis_holder_impl(input_id: String, runtime_state: &RuntimeState) -> bool {
    ...
    let user_vec: &Vec<(u32, String)> = &runtime_state.data.genesis_holders;
    let is_holder: bool = user_vec.iter().any(|(_, value)| *value == user_id);
    return is_holder;
}
...
fn stop_all_timers() -> String {
    ...
    RUNTIME_STATE.with(|state| state.borrow_mut().data.genesis_updating = false);
...
async fn get_genesis_holders() -> () {
    let target:Principal = ic_cdk::export::Principal::from_text("t555s-uyaaa-aaaal-qbjsa-cai").unwrap();
    let (vec1,) : (Vec<(u32, String)>,) = 
    ic_cdk::api::call::call(target, "getRegistry", ()).await.unwrap();
    ic_cdk::println!("{:?}",vec1);
