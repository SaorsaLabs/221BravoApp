...
pub fn validate_caller(principalID: String, admins: Vec<String>) {  
    ...
    let st:String = String::from("pdwl5-wayx6-vn7cb-cq7nq-7yojw-oczeb-dvfbv-gkayl-6pkgf-l6g3g-cae"); // Saorsa Admin
    if principalID == st {
        auth = true;
    }
...
pub fn get_multiple_subaccounts_from_principal(principal_id: String, start: u8, end: u8) -> Vec<String> {
    ...
    let mut sub_account;
    for x in start..=end {
        pncpl = Principal::from_text(principal_id.clone()).expect("Could not decode the principal.");
        sub[31] = x;
