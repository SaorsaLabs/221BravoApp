...
fn check_and_start_processing_timer(secs: u64) -> String {
    ...
    if target_set == false {
        ic_cdk::trap("Target Canisters Not Set!");
    }
...
async fn schedule_data_processing() {
    ...
    let busy = STABLE_STATE.with(|s|{
        s.borrow().as_ref().unwrap().canister_data.working_stats.is_busy.clone()
    });
    if busy == true {
        return;
    } else {
...
async fn task_manager(task_number: u8) -> bool {
    match task_number {
        0 => {
            //[][] --- Download + Process --- [][]
            let ret = download_and_process_txs().await;
            return ret;
        },
        1 => {
        ...
        _ => { 
            log("Task Manager Error :: Requested task does not exist!");
            return false;
        }
    }
}
...
fn force_reset_busy() -> String {
    ...
    // reset
    STABLE_STATE.with(|state| {
        state.borrow_mut().as_mut().unwrap().canister_data.working_stats.reset_busy()
    });
    let ret:String ="is_busy has been reset to false".to_string();
    return ret;
}
