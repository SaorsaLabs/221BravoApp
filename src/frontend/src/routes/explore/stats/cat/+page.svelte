<script>
	...
	import projectLogo from '$lib/images/projectLogos/catalyzeLogo.png';
	...
	import { CAT_decimals } from "../../../../lib/code/constants";
	...
	let decimals  = CAT_decimals;
    let vPower = 1/Math.pow(10, decimals);
	let token = "CAT";

	let socials = [
		{platform: "twitter", url: "https://twitter.com/catalyze_one"},
		{platform: "dscvr", url: "https://dscvr.one/p/catalyze"},
		{platform: "distrikt", url: "https://distrikt.app/u/Catalyze"},
		{platform: "catalyze", url: "https://catalyze.one/"}
	];
	let canisterDashboard = "https://dashboard.internetcomputer.org/sns/uly3p-iqaaa-aaaaq-aabma-cai";
	let whitepaperURL = "https://catalyze.one/whitepaper/";
	let githubURL = "https://github.com/Catalyze-Software";
	let teamWebsite = "https://catalyze.one/";
	let marketplace = "";
	let marketplace2 = "";
	let marketplace3 = "";
					...
					<tr>
						...
						<td class="box" style="width: 100%">
							<HeadCards 
								token={token} 
								tradePair={"CAT/ICP"} 
								searchURL={"/search/cat"} 
								tradeURL={"https://iclight.io/ICDex/CAT/ICP"}
								is_icrc={true}
								quoteCurrency={"icp"}
							/>
						</td>
		...
		{#if LS == true || LS == "true"} 
			<ContentBox type={"standard-shaddow-black"} style="">
				...
				{:then}
					...
					<DailyActive token={"CAT"} is_icrc={true}/>
				{/await}
			</ContentBox>
		{/if}
