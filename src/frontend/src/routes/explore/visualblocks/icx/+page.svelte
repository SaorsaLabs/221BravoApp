<script>
	...
	async function loadStuff(){
        ...
        LS = aS.data.loggedIn;
		if (LS == "true" || LS == true){
			let x = await getVisualBlockData("ICX", 0, 0, 0, 0);
			loadedTXS = x.blocks;
...
<svelte:head>
	<title>Latest Blocks: IC-X Token</title>
...
<LayoutCombine>
	...
	<span slot="head">
		<Head/>
		<VisualBlockSubHead selected="11"/>
	</span>
	...
	<span slot="body">
		...
		{:else}
			<div>
				{#await promise}
					<p class="cntr">Loading Visual Map for IC-X transactions... </p>
					<Loading/>
				{:then}
