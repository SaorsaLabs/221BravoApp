<script>
	...
	async function loadStuff(){
        ...
        LS = aS.data.loggedIn;
		if (LS == "true" || LS == true){
			let x = await getVisualBlockData("CKBTC", 0, 0, 0, 0);
			loadedTXS = x.blocks;
			...
			globalData: {
				...
				globalMoveX: 0,
				globalMoveY: 0,
				text: "ckBTC: Latest Transactions"
			}
			};
			return x;
		}
...
<LayoutCombine>
	...
	<span slot="body">
		...
		{:else}
			<div>
				{#await promise}
					<p class="cntr">Loading Visual Map for ckBTC transactions... </p>
					<Loading/>
				{:then}
