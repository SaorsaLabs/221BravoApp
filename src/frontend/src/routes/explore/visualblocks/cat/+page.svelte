<script>
	...
	async function loadStuff(){
        ...
        LS = aS.data.loggedIn;
		if (LS == "true" || LS == true){
			let x = await getVisualBlockData("CAT", 0, 0, 0, 0);
			loadedTXS = x.blocks;
			...
			globalData: {
				...
				globalMoveX: 0,
				globalMoveY: 0,
				text: "Catalyze: Latest Transactions"
			}
			};
			return x;
		}
...
<svelte:head>
	<title>Latest Blocks: Catalyze Token</title>
	<meta name="description" content="Catalyze visual block explorer" />
</svelte:head>
...
<LayoutCombine>
	...
	<span slot="body">
		...
		{:else}
			<div>
				{#await promise}
					<p class="cntr">Loading Visual Map for Catalyze transactions... </p>
					<Loading/>
				{:then}
