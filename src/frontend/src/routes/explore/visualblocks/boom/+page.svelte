<script>
	...
	async function loadStuff(){
        ...
        LS = aS.data.loggedIn;
		if (LS == "true" || LS == true){
			let x = await getVisualBlockData("BOOM", 0, 0, 0, 0);
			loadedTXS = x.blocks;
			...
			globalData: {
				...
				globalMoveX: 0,
				globalMoveY: 0,
				text: "BoomDAO: Latest Transactions"
			}
			};
			return x;
		}
...
<svelte:head>
	<title>Latest Blocks: BoomDAO Token</title>
	<meta name="description" content="BoomDAO visual block explorer" />
</svelte:head>
...
<LayoutCombine>
	...
	<span slot="head">
		<Head/>
		<VisualBlockSubHead selected="10"/>
	</span>
	...
	<span slot="body">
		...
		{:else}
			<div>
				{#await promise}
					<p class="cntr">Loading Visual Map for BoomDAE transactions... </p>
					<Loading/>
				{:then}
