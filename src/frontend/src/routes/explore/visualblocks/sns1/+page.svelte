<script>
	...
	async function loadStuff(){
		let x = await getVisualBlockData("SNS1", 0, 0, 0, 0);
		loadedTXS = x.blocks;
        ...
        globalData: {
            ...
            globalMoveX: 0,
            globalMoveY: 0,
			text: "SNS-Dragginz: Latest Transactions"
        }
        };
		return x;
	}
...
<svelte:head>
	<title>Latest Blocks: SNS-1 Token</title>
...
<LayoutCombine>
	...
	<span slot="head">
		<Head/>
		<VisualBlockSubHead selected="3"/>
	</span>

	<span slot="body">
		<div>
			{#await promise}
				<p class="cntr">Loading Visual Map for SNS-Dragginz Token... </p>
			{:then}
