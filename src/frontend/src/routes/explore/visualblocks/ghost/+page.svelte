<script>
	...
	async function loadStuff(){
        ...
        LS = aS.data.loggedIn;
		if (LS == "true" || LS == true){
			let x = await getVisualBlockData("GHOST", 0, 0, 0, 0);
			loadedTXS = x.blocks;
			...
			globalData: {
				...
				globalMoveX: 0,
				globalMoveY: 0,
				text: "GHOST: Latest Transactions"
			}
			};
			return x;
		}
...
<svelte:head>
	<title>Latest Blocks: Ghost Token</title>
...
<LayoutCombine>
	...
	<span slot="body">
		...
		{:else}
			<div>
				{#await promise}
					<p class="cntr">Loading Visual Map for Ghost transactions... </p>
					<Loading/>
				{:then}
