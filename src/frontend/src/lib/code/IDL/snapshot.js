export const snapshotIDL = ({ IDL }) => {
	...
	const QuickStats = IDL.Record({
		...
		total_unique_principals: IDL.Vec(QsRecord),
		total_principal_holders: IDL.Vec(QsRecord)
	});
	...
	const TimeStats = IDL.Record({
		...
		total_principal_holders: IDL.Nat64,
	...
	return IDL.Service({
		...
		get_quickstats: IDL.Func([IDL.Text, IDL.Nat], [IDL.Opt(QuickStats)], ['query']),
		get_standard_snapshots: IDL.Func([IDL.Text, IDL.Nat], [IDL.Opt(IDL.Vec(TimeStats))], ['query']),
