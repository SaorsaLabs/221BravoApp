...
function calculateKey() {
	let key1 = import.meta.env.VITE_AUTH_KEY_1;
	let k1 = key1.split(' ');
	let key2 = import.meta.env.VITE_AUTH_KEY_2;
	let k2 = key2.split(' ');
	let date = new Date();
	let key3 = date.getDay();
	let time = Math.floor(date.getTime() / 100000);
	let res = time * k1[key3] * k2[key3];
	return res;
}

export { stoicLogin, plugLogin, bitfinityLogin, getSync, calculateKey };
