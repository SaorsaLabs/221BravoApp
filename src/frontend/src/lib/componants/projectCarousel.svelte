<script>
    ...
    import { get_single_project } from '../code/fetchContent.js';
    import Loading from '../shared/loading.svelte';

    export let bucket = 1; 
    let pos;
    let projectArray = [];
    let promise = loadProjects();
    async function loadProjects(){
      projectArray = await get_single_project(bucket);
    ...
    <div class="card-container">
      ...
      {:then}
        {#each showArray as PR}
          <a href={PR.project_url} target="_blank">
              <div class="card">
                <!-- PR.image_url -->
                <img class="headAlign" src={PR.image_url} alt="" width="100%"/>  
                <span class="title">{PR.title}</span>
                <!-- <span>{PR.sub_title}</span> -->
              </div>
          </a>
        {/each}
      {/await}
