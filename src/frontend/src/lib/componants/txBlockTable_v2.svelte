<script>
    ...
    import TxPopupTable from '../shared/txPopupTable_v2.svelte';
        ...
        {#each currentPageRows as TX}
            <tr>
                ...
                <td>
                    <!-- FROM -->
                    <!-- ICRC Standard -->
                    {#if TX.standard == "icrc-1" || TX.standard == "icrc-2"}
                        {#if TX.fromName != undefined }
                            <!-- has name -->
                            <span class="text-info">{TX.fromName}</span>
                            {@html "<br>"}
                        {:else}
                            <!-- not named -->
                            {TX.fromShortID}
                            {#if TX.fromAccount != DEFAULT_SUBACCOUNT && TX.type != "Mint"}
                                {@html "<br>"}
                                {TX.fromShortSA}
                            {/if}
                        {/if}
                ...
                <td>   
                    ...
                    <!-- ICRC Standard -->
                    {#if TX.standard == "icrc-1" || TX.standard == "icrc-2"}
                        {#if TX.toName != undefined }
                            <!-- has name -->
                            <span class="text-info">{TX.toName}</span>
                            {@html "<br>"}
                        {:else}
                            <!-- not named -->
                            {TX.toShortID}
                            {#if TX.toAccount != DEFAULT_SUBACCOUNT && TX.type != "Burn" && TX.type != "Approve"}
                                {@html "<br>"}
                                {TX.toShortSA}
                            {/if}
                        {/if}
...
<Modal open={showPopup} title={"Transaction:"} size={"large"} onClosed={() => onPopupClose()}> 
    <TxPopupTable data={clickRowData} type={"block"}/>
</Modal>
