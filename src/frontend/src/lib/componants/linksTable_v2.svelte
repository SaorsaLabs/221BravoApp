...
<div class="mainAlign">
    ...
    <tr>
        <td>{TX.count}</td>
        <td>
            {#if TX.standard == "icp-og"}
                {#if !TX?.name}
                {TX.shortID}
                {:else}
                <span class="text-info">{TX.name}</span>
                {/if}
            {:else if TX.standard == "icrc-1" || TX.standard == "icrc-2"}
                {#if !TX?.name}
                    {TX.shortID}
                    {#if TX.splitSA != DEFAULT_SUBACCOUNT}
                        {@html "<br>"} 
                        {TX.shortSA}
                    {/if}
                {:else}
...
<Modal open={showPopup} title={"Linked Account:"} size={"large"} onClosed={() => onPopupClose()}> 
    {#if clickRowData.standard == "icp-og"}
    <span class="fontSpan">
        {clickRowData.ID}
        <a href="/search/token/{clickRowData.token}?id={clickRowData.ID}&sub=''" target="_blank"> <img class="search" src={search} alt="search" width="20px" style="margin-left:5px"/> </a>
    ...
    {:else if clickRowData.standard == "icrc-1" || clickRowData.standard == "icrc-2"}
        <span class="fontSpan textAdj">
            Principal :
            {clickRowData.splitID}
            <a href="/search/token/{clickRowData.token}?id={clickRowData.splitID}&sub={clickRowData.splitSA}" target="_blank"> <img class="search" src={search} alt="search" width="20px" style="margin-left:5px"/> </a>
            <CopyButton text={clickRowData.ID}/>
            <SaveButton text={clickRowData.splitID} text2={clickRowData.splitSA} icrcAccount={true}/>
            {@html "<br>"}  
                {#if clickRowData?.shortSA != undefined}
                Sub-Account :
