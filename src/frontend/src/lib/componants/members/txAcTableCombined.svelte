<script>
	...
	const tableHeaders = ['#','Date/Time','token','Account','Direction','Value','info'];
    const dataNames = ['counter', 'date', 'time', 'shortID', 'direction', 'value', 'hash', 'block', 'longID', 'token'];
	let selectedHeader = "#";
	...
	const sortByNumber = (colHeaderInput) => {
        ...
        else if (colHeaderInput == tableHeaders[2]) dataNM = dataNames[9]; 
        else if (colHeaderInput == tableHeaders[3]) dataNM = dataNames[8];
        else if (colHeaderInput == tableHeaders[4]) dataNM = dataNames[4];
        else if (colHeaderInput == tableHeaders[5]) dataNM = dataNames[5];
        else if (colHeaderInput == tableHeaders[6]) dataNM = dataNames[0];
