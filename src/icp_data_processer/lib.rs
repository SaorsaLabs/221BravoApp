...
// #[update]
// async fn manual_call(start: u64, length: u64) -> String {
//     let x = icp_transaction_download(start, length).await;
//     match x {
//         Some(v) => {
//             log(format!("TRANSACTIONS: {:?}", v));
//         },
//         _ => {
...
//     //process_data().await;
//     return "Call Complete".to_string();
// }
...
async fn process_data() {
    ...
    let hour_start_time: u64 = with_runtime(|rts|{ 
        (time_now as u128 - (rts.data.canister_settings.hours_to_calculate as u128 * HOUR_AS_NANOS as u128)) as u64
    });
    let day_start_time: u64 = with_runtime(|rts|{ 
        (time_now as u128 - (rts.data.canister_settings.days_to_calcualte as u128 * DAY_AS_NANOS as u128)) as u64
    });
    ...
    match ledger_id {
        Ok(ledger_id) => {
            let tip_chain = get_tip_of_chain(ledger_id)
                                .await
                                .map_err(|err_string| format!("Can't fetch tip of chain : {}", err_string))
                                .unwrap();
            
            let tip_u128  = tip_chain
                                .to_u128()
                ...
                if tip > 0 {
                    ...
                    if blocks_needed > 0 {
                        ...
                        for i in 0..chunks_needed {
                            ...
                            // Get transactions
                            let txns: Option<Vec<ProcessedTX>> = icp_transaction_download(start as u64, length as u64).await;
...
async fn icp_transaction_download(start: u64, length: u64) -> Option<Vec<ProcessedTX>> { 
    ...
    let res: Result<QueryBlocksResponse, String> = 
            get_transactions_from_ledger(ledger_id, start, length)
            .await;
        match res {
            Ok(value) => {
                                                    ...
                                                    Ok(v) => {
                                                        ...
                                                        for block_data in v.blocks {
                                                            match block_data.transaction.operation {
                                                                    Some(value) => {
                                                                        ...
                                                                        match value {
                                                                            OperationEnum::Burn { from, amount } => {
                                                                                ...
                                                                                processed_transactions.push(process_burn_transaction(input, &block, &block_data.timestamp.timestamp_nanos, &hash));
                                                                                block+= Nat::from(1);
                                                                            },
                                                                            OperationEnum::Mint { to, amount } => {
                                                                                let input = (hex::encode(to), amount.e8s);
                                                                                processed_transactions.push(process_mint_transaction(input, &block, &block_data.timestamp.timestamp_nanos, &hash));
                                                                                block+= Nat::from(1);
                                                                            },
                                                                            OperationEnum::Transfer { from, to, amount, fee } => {
                                                                                let input = (hex::encode(from), hex::encode(to), amount.e8s);
                                                                                processed_transactions.push(process_transfer_transaction(input, &block, &block_data.timestamp.timestamp_nanos, &hash));
                                                                                block+= Nat::from(1);
                                                                            },
                                                                    ...
                                                                    _ => {
                                                                        log(format!("Transaction does not contain operation! Block: {}", block));
                                                                    },
                                                            }
                                                        }
                                                    },
                                                    Err(error) => {
                                                        log(format!("Error trying to unwrap Archive blocks. Err : {:?}", error));
                                                    },
                                                }
                                            },
                                            Err(err_text) => {
                            ...
                            match block_data.transaction.operation {
                                Some(value) => {
                                    ...
                                    match value {
                                        OperationEnum::Burn { from, amount } => {
                                            let input = (hex::encode(from), amount.e8s);
                                            processed_transactions.push(process_burn_transaction(input, &block_master, &block_data.timestamp.timestamp_nanos, &hash));
                                            block_master+= Nat::from(1);
                                        },
                                        OperationEnum::Mint { to, amount } => {
                                            let input = (hex::encode(to), amount.e8s);
                                            processed_transactions.push(process_mint_transaction(input, &block_master, &block_data.timestamp.timestamp_nanos, &hash));
                                            block_master+= Nat::from(1);
                                        },
                                        OperationEnum::Transfer { from, to, amount, fee } => {
                                            let input = (hex::encode(from), hex::encode(to), amount.e8s);
                                            processed_transactions.push(process_transfer_transaction(input, &block_master, &block_data.timestamp.timestamp_nanos, &hash));
                                            block_master+= Nat::from(1);
                                        },
                                        OperationEnum::Approve {
                                            fee,
                                        ...
                                        OperationEnum::TransferFrom {
                                            ...
                                            log(format!("Transfer from operation. Block: {}", block_master));
                                        },
                                    }
                                }
                                _ => {
...
async fn get_transactions_from_ledger(ledger_id: Principal, start: u64, length: u64,) -> 
    ...
    let (res,): (QueryBlocksResponse,) =
        ic_cdk::call(ledger_id, "query_blocks", (req,))
            .await
            .map_err(|(code, str)| format!("code: {:#?} message: {}", code, str))?;
    Ok(res)
}

async fn get_transactions_from_archive(archived: &ArchivedBlocksRange,) -> 
    ...
    let (res,): (GetBlocksResult,) =
        ic_cdk::call(ledger_id, method, (req,))
            .await
            .map_err(|(code, str)| format!("code: {:#?} message: {}", code, str))?;
    Ok(res)
}

fn process_mint_transaction(data: (String, u64), block: &Nat, timestamp: &u64, hash: &String) -> ProcessedTX {   
...
fn process_burn_transaction(data: (String, u64), block: &Nat, timestamp: &u64, hash: &String) -> ProcessedTX {
...
fn process_transfer_transaction(data: (String, String, u64), block: &Nat, timestamp: &u64, hash: &String) -> ProcessedTX {
                ...
                match tx.tx_type.as_str() {
                    "Transaction" => {
                        ...
                        if let Some(ac) = data.account_holders.get(&tx.from_account) {
                            ...
                            if ac.balance < (tx_value_u128 + tx_fee) {
                                ...
                                log(format!("Caught overflow from transfer. Account: {}", &tx.from_account));
                            } else {
                            ...
                            log(format!("Error: Sent transaction from new unknown account {}", &tx.from_account));
                            processed_ok = false;
                        }
                    ...
                    "Burn" => {
                        ...
                        if let Some(ac) = data.account_holders.get(&tx.from_account) {
                            ...
                            log(format!("Error: Burn transaction from new account {}", tx.from_account));
                        }
                    },
                    _ => {
